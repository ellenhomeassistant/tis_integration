...
class TISLight(LightEntity):
    def __init__(A,tis_api,gateway,light_name,channel_number,device_id):A.api=tis_api;A.gateway=gateway;A.device_id=device_id;A.channel_number=int(channel_number);A._attr_name=light_name;A._attr_state=_D;A._attr_brightness=_A;A.listener=_A;A._attr_unique_id=beta__("e19fdmFyMH1fe19fdmFyMX0=", __var0=A.name, __var1=A.channel_number);A.setup_light()
    ...
    async def async_added_to_hass(A):
        ...
        async def B(event):
            ...
            if B.event_type==str(A.device_id):
                ...
                elif B.data[_B]==alpha__("YmluYXJ5X2ZlZWRiYWNr"):
...
class TISRGBLight(LightEntity):
    def __init__(A,tis_api,gateway,device_id,r_channel,g_channel,b_channel,light_name):A.api=tis_api;A.gateway=gateway;A.device_id=device_id;A.r_channel=int(r_channel);A.g_channel=int(g_channel);A.b_channel=int(b_channel);A.rgb_value_flags=[0,0,0];A._attr_name=light_name;A._attr_state=_A;A._attr_rgb_color=_A;A.listener=_A;A._attr_unique_id=beta__("e19fdmFyMH1fe19fdmFyMX1fe19fdmFyMn1fe19fdmFyM30=", __var0=A.name, __var1=A.r_channel, __var2=A.g_channel, __var3=A.b_channel);A.setup_light()
    ...
    async def async_added_to_hass(A):
        ...
        async def B(event):
            ...
            if B.event_type==str(A.device_id):
                ...
                elif B.data[_B]==_G:E=B.data[_C];F=E[A.r_channel]/100*255;G=E[A.g_channel]/100*255;H=E[A.b_channel]/100*255;I=E[A.w_channel]/100*255;A._attr_rgbw_color=F,G,H,I;A._attr_state=bool(F or G or H or I)
                elif B.data[_B]==_H:A._attr_state=STATE_UNKNOWN
        A.listener=A.hass.bus.async_listen(str(A.device_id),B)
        for C in range(5):
