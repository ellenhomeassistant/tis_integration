...
_H=alpha__("bWRpOmN1cnJlbnQtYWM=")
_G=alpha__("bWRpOnRoZXJtb21ldGVy")
_F=alpha__("aGVhbHRoX3NlbnNvcg==")
_E=alpha__("dGVtcF9zZW5zb3I=")
_D=alpha__("bW9udGhseV9lbmVyZ3lfc2Vuc29y")
_C=alpha__("YW5hbG9nX3NlbnNvcg==")
...
async def async_setup_entry(hass,entry,async_add_devices):
    A=hass;B=entry.runtime_data.api;I=[]
    for(J,D)in RELEVANT_TYPES.items():
        K=await B.get_entities(platform=J)
        if K and len(K)>0:
            ...
            for(E,F,G,P,H,min,max)in L:
                if J==_C:C.append(D(hass=A,tis_api=B,gateway=H,name=E,device_id=G,channel_number=F,min=min,max=max))
                elif J==_A:
                    for(M,N)in ENERGY_SENSOR_TYPES.items():C.append(D(hass=A,tis_api=B,gateway=H,name=beta__("e19fdmFyMH0ge19fdmFyMX0=", __var0=N, __var1=E),device_id=G,channel_number=F,key=M,sensor_type=_A))
                    C.append(D(hass=A,tis_api=B,gateway=H,name=beta__("TW9udGhseSBFbmVyZ3kge19fdmFyMH0=", __var0=E),device_id=G,channel_number=F,sensor_type=_D))
                else:C.append(D(hass=A,tis_api=B,gateway=H,name=E,device_id=G,channel_number=F))
            I.extend(C)
...
def get_coordinator(hass,tis_api,device_id,gateway,coordinator_type,channel_number):
    ...
    if E not in coordinators:
        ...
        elif A==_D:C=protocol_handler.generate_update_monthly_energy_packet(entity=B)
...
class CoordinatedTemperatureSensor(BaseSensorEntity,SensorEntity):
    def __init__(A,hass,tis_api,gateway,name,device_id,channel_number):C=channel_number;B=device_id;D=get_coordinator(hass,tis_api,B,gateway,_E,C);super().__init__(D,name,B);A._attr_icon=_G;A.name=name;A.device_id=B;A.channel_number=C;A._attr_unique_id=beta__("c2Vuc29yX3tfX3ZhcjB9", __var0=A.name)
...
class CoordinatedLUXSensor(BaseSensorEntity,SensorEntity):
    def __init__(A,hass,tis_api,gateway,name,device_id,channel_number):C=channel_number;B=device_id;D=get_coordinator(hass,tis_api,B,gateway,_F,C);super().__init__(D,name,B);A._attr_icon=alpha__("bWRpOmJyaWdodG5lc3MtNg==");A.name=name;A.device_id=B;A.channel_number=C;A._attr_unique_id=beta__("c2Vuc29yX3tfX3ZhcjB9", __var0=A.name)
    async def async_added_to_hass(A):
        await super().async_added_to_hass()
        @callback
        def B(event):
...
class CoordinatedAnalogSensor(BaseSensorEntity,SensorEntity):
    def __init__(A,hass,tis_api,gateway,name,device_id,channel_number,min=0,max=100):C=channel_number;B=device_id;D=get_coordinator(hass,tis_api,B,gateway,_C,C);super().__init__(D,name,B);A._attr_icon=_H;A.name=name;A.device_id=B;A.channel_number=C;A.min=min;A.max=max;A._attr_unique_id=beta__("c2Vuc29yX3tfX3ZhcjB9", __var0=A.name)
    async def async_added_to_hass(A):
        await super().async_added_to_hass()
        @callback
        def B(event):
...
class CPUTemperatureSensor(SensorEntity):
    def __init__(A,hass):A._cpu=CPUTemperature();A._state=A._cpu.temperature;A._hass=hass;A._attr_name=alpha__("Q1BVIFRlbXBlcmF0dXJlIFNlbnNvcg==");A._attr_icon=_G;A._attr_update_interval=timedelta(seconds=10);A._attr_unique_id=beta__("c2Vuc29yX3tfX3ZhcjB9", __var0=A.name);async_track_time_interval(A._hass,A.async_update,A._attr_update_interval)
